---
title: "nanostring_geomx_proteogenomics_WTAanalysis_SEPT6GOF"
...
#loading the data
```{r}
datadir <- file.path("/Users/ericallenspach/r_projects/nanostring_rna_sept6/20230331_Allenspach_AllSeq_sept6")
```

```{r}
# automatically list files in each directory for use
DCCFiles <- dir(here("20230331_Allenspach_AllSeq_sept6","dccs"), pattern = ".dcc$",
                full.names = TRUE, recursive = TRUE)
PKCFiles <- dir(here("20230331_Allenspach_AllSeq_sept6","pkcs"), pattern = ".pkc$",
                                full.names = TRUE, recursive = TRUE)
SampleAnnotationFile <- dir(here("20230331_Allenspach_AllSeq_sept6", "annotation"), pattern = ".xlsx$", full.names = TRUE, recursive = TRUE)
```
...
pData(target_ourData)[["slide"]] <-
    factor(pData(target_ourData)[["slide name"]])

# run LMM:
# formula follows conventions defined by the lme4 package
results2 <- c()
mixedOutmc <- mixedModelDE(target_ourData,
                           elt = "log_q",
                           modelFormula = ~ group + (1 | slide),
                           groupVar = "group",
...
top_g <- c()
for(cond in c("Control Bone Marrow B", "SEPT6 GOF")) {
...
#We can review our genes of interest for this comparison as well.
library(dplyr)
kable(subset(results2, Gene %in% GOI & Color == "P < 0.05"), digits = 3,
...
ggplot(results2,
    ...
    geom_text_repel(data = subset(results2, Gene %in% GOI & FDR < 0.001),
